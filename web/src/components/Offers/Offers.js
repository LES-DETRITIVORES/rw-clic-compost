...
const Offers = ({ meals, onOffer, onService, onRate, defaultValue }) => {
  ...
  const offers = [
    {
      ...
      get qty() {
        return Math.max(
          Math.round((meals * LITER_BY_MEAL) / this.liter),
          meals == 0 ? 1 : 2
        )
      },
      get rate() {
    ...
    },
    {
      ...
      get qty() {
        return Math.round((meals * LITER_BY_MEAL) / this.liter)
      },
      get rate() {
        return parseFloat((this.qty * this.liter * PRICE_BY_LITER).toFixed(2))
      },
  ...
  const [index, setIndex] = defaultValue
    ? useState(defaultValue)
    : useState(bestOffer(offers))
                          ...
                          >
                            ...
                            {meals == 0 && (
                              <span className="block font-bold">
                                {(offer.rate * 1.2).toFixed(2)}€ TTC / semaine
                              </span>
                            )}
                            {meals > 0 && (
                              <span className="block font-bold">
                                {offer.rate.toFixed(2)}€ HT / semaine (
                                {(offer.rate / meals).toFixed(2)} € HT / repas)
                              </span>
                            )}
                          </RadioGroup.Description>
                        </div>
