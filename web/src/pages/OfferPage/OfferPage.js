          ...
          <div className="grid grid-cols-1 gap-8 md:grid-cols-2"> 
            <div>
              <div className="mt-8 text-lg">
                <Offers meals={profile=="particulier" ? 0 : meals} onOffer={setOffer} onService={setService} onRate={setRate} defaultValue={offer}/>
              </div>
            </div>
            <div>
              <Form onSubmit={offerSubmit} className="container mx-auto font-sans">
                ...
                <div>
                    <Submit
                      className={`sm:text-sm md:text-lg uppercase font-bold ${offer && firstname && lastname && email && phone ? 'bg-orange-600' : 'bg-gray-600'}  rounded-b-md p-4 text-white w-full shadow-lg`}
                      disabled={!(offer && firstname && lastname && email && phone)}>
